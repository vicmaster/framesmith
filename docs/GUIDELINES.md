...
- **`canvas_evaluate`** scores the design on 6 categories (spacing, color, typography, structure, consistency, cliche) and surfaces actionable issues with `nodeId` references. Use it in a generator-evaluator loop: `batch_design` → `canvas_evaluate` → fix the returned nodeIds.
