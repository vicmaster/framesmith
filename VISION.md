...
- [x] Input primitives — `toggle`, `checkbox`, `radio`, `select` node types with `checked`/`value`/`disabled`, styled from `$tokens` with neutral fallbacks
...
_Full spec-driven breakdown in [`docs/specs/PHASE-17-SPEC.md`](docs/specs/PHASE-17-SPEC.md). Phase 17 complete (v1.4.0)._
...
- [x] Grid + centered containers — computed `grid-template-columns` → rows of proportional columns (spans honored); `margin: auto` / `max-width` / flex-center → centered frames at their real width instead of full-bleed stretches
- [x] Geometry-clustering fallback + `report.layout` — children's computed rects cluster into row/column structure when no semantic pass applies; every reconstruction (or honest `stack-fallback`) recorded in the report
...
Machine critique flows to the agent as structured data; human critique arrives as prose the agent must re-ground by guessing ("the third card — no, the other one"). The viewer already knows which node every pixel belongs to (`data-node-id` on every rendered element, same-origin iframes) — let the human click an element, type a note, and have it land as `{ nodeId, comment }` the agent reads through a tool. The inverse of `canvas_evaluate`: point-and-tell instead of describe-and-hope.
...
- [x] Loop integration — `openFeedback` count on `canvas_list`/`canvas_evaluate` (+ `init` workspace total); the evaluate `directive` stays blocking while feedback is open — the operating contract: open feedback blocks present, same bar as open inspector comments
...
- [x] Slice A — rendering primitives: per-side borders (`borderTop`…, style solid/dashed/dotted) + `strokeDasharray` on paths (#131, #132)
...
- [x] Slice D — agent surfaces: drift-check-before-designing workflow in INSTRUCTIONS/GOTCHAS/GUIDELINES/README + discoverability guards
...
- [x] Slice F — agent surfaces: "derive, don't hand-pick" workflow + discoverability pins

_Full spec-driven breakdown in [`docs/specs/PHASE-25-SPEC.md`](docs/specs/PHASE-25-SPEC.md). All slices shipped — Phase 25 complete. Phase 26 ("Beyond one canvas") takes the cluster's remainder: grid authoring, project-level consistency, multi-screen flow critique._
